...
const NotificationBell = ({ isCollapsed }) => {
  ...
  const markAllAsRead = async () => {
    if (!user) return;
    try {
      await api.patch(`/api/v1/notificaciones/read-all/${user.id}`);
      setNotifications([]);
    } catch (error) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-3 p-2.5 rounded-xl hover:bg-black/5 transition-all relative group ${isCollapsed ? 'justify-center w-10 h-10' : 'w-full px-4'
          }`}
      >
        ...
        {unreadCount > 0 && (
          <span className={`absolute bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white shadow-sm ${isCollapsed ? 'top-1 right-1 px-1.5 py-0.5 min-w-[18px] h-[18px]' : 'right-4 px-2 py-0.5'
            }`}>
      ...
      {showDropdown && (
        <div className={`absolute ${isCollapsed ? 'left-14 bottom-0' : 'left-0 bottom-full mb-2'} w-80 bg-white shadow-xl rounded-xl z-[100] border border-black/5 overflow-hidden animate-in fade-in zoom-in duration-200`}>
          ...
          {notifications.length > 0 && (
            <div className="p-3 flex justify-between items-center border-t border-black/5 bg-surface-container-lowest px-4">
              <button
                className="text-[10px] text-green-600 hover:text-green-700 font-bold uppercase tracking-tighter transition-colors cursor-pointer"
                onClick={markAllAsRead}
              >
                Marcar todas como leídas
              </button>
              <button
                className="text-[10px] text-primary/60 hover:text-primary font-bold uppercase tracking-tighter transition-colors cursor-pointer"
