...
const MyTutorsHistory = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const sortedSessions = (sessionsData || []).sort(
        (a, b) => new Date(b.programadaPara) - new Date(a.programadaPara),
      );
      ...
      const solicitudesActivas = (solicitudesData || []).filter(
        (s) =>
          (s.estado === "pendiente" || s.estado === "aceptada") && !s.pagada,
      );
  ...
  const getStatusBadge = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case "pendiente":
        return {
          text: "⏳ Pendiente de Tutor",
          className: "bg-amber-50 text-amber-700 border border-amber-200",
        };
      case "aceptada":
        return {
          text: "💳 Aceptada (Por Pagar)",
      ...
      <main className="p-4 md:p-10 min-h-screen">
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="max-w-2xl">
            <span className="text-xs font-bold text-academic-gold uppercase tracking-widest mb-2 block">
              Académico
            </span>
            <h1 className="text-3xl md:text-5xl font-extrabold text-primary tracking-tight font-display">
              Mi Historial Académico
            </h1>
            <p className="text-gray-500 mt-2 text-sm">
              Gestiona tus solicitudes enviadas y consulta el historial de tus
              encuentros.
            </p>
          </div>
            ...
            {filteredSessions.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <p className="text-gray-500 text-sm">
                  No posees registros de tutoría que coincidan con la búsqueda.
                </p>
              </div>
            ) : (
                ...
                <thead className="bg-surface-container-low border-b border-outline-variant/10">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="px-8 py-5 text-[10px] font-bold uppercase tracking-widest text-elegant-gray"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {filteredSessions.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-semibold text-primary">
                              {row.tutorNombre || "Tutor Académico"}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-8 py-6">
                          <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider ${badge.className}`}
                          >
                            {badge.text}
                          </span>
                        </td>
                        <td className="px-8 py-6 text-sm">
                          {row.estado?.toLowerCase() === "programada" ||
                          row.estado?.toLowerCase() === "en_progreso" ? (
                            row.enlaceReunion ? (
                              <a
                              ...
                              <span className="text-xs text-elegant-gray italic">
                                Enlace pendiente
                              </span>
                            )
                          ) : row.motivoCancelacion ? (
                            <span
                              className="text-xs text-red-700 bg-red-50 px-2.5 py-1 rounded border border-red-100 italic block max-w-xs truncate"
                              title={row.motivoCancelacion}
                            >
          ...
          <section className="space-y-4 max-w-4xl">
            {solicitudes.length === 0 ? (
              <div className="bg-surface-container-lowest border border-outline-variant/10 rounded-xl p-12 text-center shadow-sm">
                ...
                <p className="text-gray-500 text-sm">
                  Aquí se listarán las propuestas que envíes a los tutores y
                  estén esperando gestión o pago.
                </p>
              </div>
            ) : (
              solicitudes.map((sol) => {
                ...
                return (
                  <div
                    key={sol.id}
                    className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/10 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-outline-variant/30 transition-all"
                  >
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-bold text-primary">
                          Tutor: {sol.tutorNombre || "Por Asignar"}
                        </span>
                        <span className="text-xs text-elegant-gray">•</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-elegant-gray">
                        ...
                        <p>
                          ⏰ <strong>Hora:</strong>{" "}
                          {sol.horaPreferida.substring(0, 5)} ({sol.duracionMin}{" "}
                          min)
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-4 shrink-0 self-end md:self-center">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider ${badge.className}`}
                      >
                        {badge.text}
                      </span>
