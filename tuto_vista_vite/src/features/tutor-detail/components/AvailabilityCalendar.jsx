    ...
    <div className="bg-surface-container-lowest rounded-3xl p-5 md:p-10 shadow-ambient border border-outline-variant/10">
      <div className="mb-8">
        <h4 className="font-headline font-bold text-xl text-primary mb-2">
          Espacios de Disponibilidad Semanal
        </h4>
        <p className="text-sm text-elegant-gray font-display">
          Selecciona un bloque de horario para agendar tu tutoría. Los bloques
          azules (⭐) son horarios especiales añadidos por el tutor.
        </p>
      </div>
          ...
          return (
            <div key={dayIdx} className="space-y-3">
              ...
              {hasSlots ? (
                daySlots.map((slot) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full py-3 px-2 rounded-xl text-xs font-bold transition-all text-center border font-display ${
                        isSelected
                ...
                <span className="text-[10px] text-gray-400 italic block text-center py-2 font-display">
                  No disp.
                </span>
              )}
            </div>
      ...
      {selectedSlot && (
        <form
          ...
          className="mt-8 pt-8 border-t border-outline-variant/10 space-y-6 animate-in fade-in duration-500"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-surface-container-low p-6 rounded-2xl mb-6">
            <div>
              <h5 className="font-bold text-xs text-primary uppercase tracking-wider mb-3 font-headline">
                Tu Bloque Seleccionado
              </h5>
              <p className="text-sm text-elegant-gray font-display">
                Día:{" "}
                <span className="font-bold text-primary">
                  {FULL_DAY_NAMES[selectedSlot.diaSemana]}
                </span>
              </p>
              <p className="text-sm text-elegant-gray font-display">
                Rango Permitido:{" "}
                <span className="font-bold text-primary">
                  {selectedSlot.horaInicio.substring(0, 5)} -{" "}
                  {selectedSlot.horaFin.substring(0, 5)}
                </span>
              </p>
            </div>
              ...
              {compromisosDelDia.length > 0 ? (
                <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                  {compromisosDelDia.map((comp, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center text-xs bg-white px-3 py-1.5 rounded-lg border border-gray-100 shadow-sm font-display"
                    >
                      <span className="font-mono font-bold text-gray-700">
                        {comp.horaInicio} - {comp.horaFin}
                      </span>
                      <span
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-elegant-gray block font-display">
                Materia de estudio *
              </label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                required
                className={`w-full px-4 py-3 bg-surface-container-low border rounded-xl text-sm font-medium focus:outline-none transition-colors font-display ${
                  infoConflictoCompromiso
                    ? "border-red-300 focus:border-red-500 bg-red-50/30"
                    : "border-outline-variant/10 focus:border-academic-gold"
                }`}
              />
            </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-elegant-gray block font-display">
              Mensaje / Dudas o temas a tratar
            </label>
            <textarea
              rows="3"
              value={message}
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl flex items-start gap-3 my-4 animate-in fade-in duration-300">
              ...
              <div className="text-xs font-medium font-display">
                <p className="font-bold text-amber-900 mb-0.5">
                  Horario no disponible para esta fecha
                </p>
                El tutor ha inhabilitado la franja de{" "}
                <span className="font-bold">
                  {infoBloqueo.inicio} a {infoBloqueo.fin}
                </span>{" "}
                para el día elegido. Por favor, selecciona una hora diferente
                fuera de ese rango.
              </div>
            </div>
          )}
          ...
          {infoConflictoCompromiso && (
            <div className="p-4 bg-red-50 border border-red-200 text-red-800 rounded-xl flex items-start gap-3 my-4 animate-in fade-in duration-300">
              ...
              <div className="text-xs font-medium font-display">
                ...
                Tu tutoría propuesta se cruza con una{" "}
                <span className="font-bold text-red-900 uppercase">
                  "{infoConflictoCompromiso.label}"
                </span>{" "}
                agendada de{" "}
                <span className="font-bold">
                  {infoConflictoCompromiso.horaInicio} a{" "}
                  {infoConflictoCompromiso.horaFin}
                </span>
                . Ajusta los minutos o la hora de inicio.
              </div>
            </div>
          )}
          ...
          <div className="pt-6 border-t border-outline-variant/10 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              ...
              <p className="text-2xl md:text-3xl font-black text-primary font-headline">
                ...
                <span className="text-sm font-medium text-gray-400 font-display">
                  {" "}
                  / h
                </span>
              </p>
