...
export const MessageBubble = ({ text, timestamp, isSent, isRead, imageUrl }) => {
  return (
    // 1. AÑADIDO 'min-w-0' AQUÍ: Esto le quita el límite de encogimiento a Flexbox
    <div className={`flex flex-col w-full min-w-0 ${isSent ? 'items-end' : 'items-start'}`}>

      {/* Texto del Mensaje */}
      {text && (
        // 2. ELIMINADO 'w-fit', mantenemos solo el ancho máximo
        <div className={`max-w-[85%] md:max-w-2xl p-4 md:p-6 rounded-xl ${isSent ? 'rounded-tr-none' : 'rounded-tl-none'} shadow-sm ${isSent
          ...
          {/* 3. CAMBIADO 'break-words' por 'break-all' para forzar el corte de la URL */}
          <p className="text-sm leading-relaxed font-body break-all whitespace-pre-wrap">
            {text}
          </p>
        </div>
      )}
